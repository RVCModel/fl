...
function safeUrlTransform(url: string) {
  ...
  if (!lowered) return ""
  if (lowered.startsWith("#")) return value
  if (lowered.startsWith("/")) return value
  if (lowered.startsWith("http://") || lowered.startsWith("https://")) return value
  if (lowered.startsWith("mailto:") || lowered.startsWith("tel:")) return value
  return ""
}
      ...
      <div className="relative last:before:absolute last:before:h-full last:before:w-4 last:before:bg-background">
        <CollapsibleTrigger className="group/experience not-prose block w-full select-none text-left">
          <div className="relative z-1 mb-1 flex items-center gap-3 bg-background">
            ...
            <h4 className="flex-1 text-balance text-base font-medium">{position.title}</h4>
        ...
        <CollapsibleContent className="overflow-hidden duration-300 data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
          {position.description && (
            <Prose className="pt-2 pl-9">
              <ReactMarkdown urlTransform={safeUrlTransform}>{position.description}</ReactMarkdown>
            </Prose>
          )}
